...
def Detail_model(state: MessagesState):
    """Call the LLM to generate answer for user prompt"""
    response = llm.invoke(
        [
            {"role": "system", "content": "Answer user query like a professional"},
            *state["messages"]
        ]
    )
    return {"messages": [response]}
...
StateDetailGraph = (
    ...
    .add_node("Detail_model", Detail_model)
    .add_edge(START, "Detail_model")
)


StateDetailAgent = StateDetailGraph.compile(checkpointer=pg_checkpointer)

class AiStateDetailView(APIView):
    authentication_classes = [] 
    permission_classes = [AllowAny]
    ...
    def get(self, request):
        ...
        try:
            config = {"configurable": {"thread_id": thread_id}}
            state = StateDetailAgent.get_state(config)
            history = list(StateDetailAgent.get_state_history(config))

            return Response(
                {
                    "current_state": state.values if state else None,
                    "history": [h.values for h in history]
                },
                status=status.HTTP_200_OK
            )
    ...
    def post(self, request):
        user_query = request.data.get('prompt')
        thread_id = request.data.get('thread_id', 'default_thread-1')
        ...
        config = {"configurable": {"thread_id": thread_id}}
        try:
            ai_state = StateDetailAgent.invoke(
                {"messages": [{"role": "user", "content": user_query}]},
                config=config
            )
            
            final_answer = ai_state["messages"][-1].content
            return Response({"message": final_answer}, status=status.HTTP_200_OK)
