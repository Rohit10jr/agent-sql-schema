...
agent = create_agent(
    model=llm, tools=[web_search, get_weather], system_prompt="You are a helpful assistant"
)
...
def Node1(state: StreamState):
    ...
    writer("Pirate node")
    msg = llm.invoke(
        f"Be a very polite, grumpy, sarcastic pirate. Write a short joke about: {user_query}"
    )
...
def Node2(state: StreamState):
    ...
    writer("Robot node")
    msg = llm.invoke(
        f"Be a very polite, grumpy, sarcastic robot. Write a short joke about: {user_query}"
    )
            ...
            try:
                ...
                for chunk in agent.stream(
                    ...
                    for node_name, data in chunk.items():
                        ...
                        payload = json.dumps({"node": node_name, "update": serializable_update})
        ...
        def stream_generator():
            try:
                for token, metadata in stream_agent.stream(
                    ...
                    if content:
                        payload = json.dumps(
                            {"node": metadata.get("langgraph_node", "unknown"), "text": content}
                        )
                        yield f"data: {payload}\n\n"
                        ...
                        if isinstance(token, AIMessageChunk):
                            ...
                            if text:
                                payload = {
                                    "type": "token",
                                    "text": text,
                                    "tool_chunk": token.tool_call_chunks
...
guard_agent = create_agent(model=llm, tools=[get_weather], middleware=[safety_guardrail])
        ...
        def stream_generator():
            try:
                ...
                for stream_mode, data in human_loop_agent.stream(
                    input_data, stream_mode=["messages", "updates"], config=config
                ):
                    # --- 1. LIVE TOKENS ---
                    if stream_mode == "messages":
                        token, metadata = data
